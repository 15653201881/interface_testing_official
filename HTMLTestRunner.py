...
class HTMLTestRunner(Template_mixin):
    ...
    def getReportAttributes(self, result):
        ...
        if status:
            status = ' '.join(status)
            passrate = str("%.2f%%" % (float(result.success_count) / float(result.success_count + result.failure_count + result.error_count + result.skip_count) * 100))
        else:
            status = 'none'
        ...
        return [
            ...
            {"name": "状态", "value": status + " 通过率 = "+ passrate},
        ]
    ...
    def sender(self, to=None, subject=None, contents=None, attachments=None):
        ...
        if contents is None:
            contents = env.get_template('mail.html').render(
                #mail_tester = str(Report_Attri[0]),
                mail_startTime = str(Report_Attri[0]['value']),
                mail_Duration = str(Report_Attri[1]['value']),
                mail_Result = str(Report_Attri[2]['value']),
